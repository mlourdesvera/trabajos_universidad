...
# ¿Existe alguna relación entre el pH de los vinos (pH) y alguna de las otras variables? no 
...
ax.set_title('Relación pH vs acidez') # Titulo del gráfico
ax.set_xlabel('Valor del pH', fontsize='medium')   # Nombre eje X           
ax.set_ylabel('Acidez (g/dm3)', 
              fontsize='medium')                     # Nombre eje Y
#grafico2
fig, ax = plt.subplots() 
plt.rcParams['font.family'] = 'sans-serif'           
ax.scatter(data = wine,  
           ...
           y='sulphates',
           s=8,                       # Tamano de los puntos
           color='#66b3ff')           # Color de los puntos

ax.set_title('Relación pH vs sulfatos') # Titulo del gráfico
ax.set_xlabel('Valor del pH', fontsize='medium')   # Nombre eje X           
...
ax.scatter(data = wine,  
           ...
           y='alcohol',
           s=8,                       # Tamano de los puntos
           color='#013220')           # Color de los puntos

ax.set_title('Relación pH vs alcohol') # Titulo del gráfico
...
# ¿Cuál fue su objetivo: Explorar, Explicar, Otro? explorar
# ¿Qué tipos de variables estaban en juego? cuantitativas por eso este gráfico
...
# Genera el grafico de barras de las ventas mensuales (grafico por defecto) no que muy lindo por defecti
fig, ax = plt.subplots()
...
ax.set_xlim(0, 11)     #limita valores de los ejes
ax.set_ylim(0, 250)     #limita valores de los ejes

ax.set_xticks(range(1,11,1))               # Muestra todos los ticks del eje x (ticks=numeritos de abajo/costado)
...
#serie temporal- gráfico de barras
...
cheetahRegion.plot(x='Anio', 
                    ...
                    ax = ax)
#si quiero un color, añado color= y le doy una lista de colores, que corresponden a label
...
#dos grafico de barras en el mismo  eje
#%%
##### 7. GRAFICO DE LINEAS
...
ax.plot('Anio','Ventas', data=cheetahRegion, marker="o")   #puedo poner lo que quiero de marker
...
# Muestra la leyenda
ax.legend()
#muy usados en tendencias en series temporales
#%%
#############################################################################
#                                                                           #
#                              MTCARS                                       #
...
sns.clustermap(df) #no esta proporcionado. con la escala que tengo, muy amplia y puede ser menor a otro atributo, por eso estandarizamos
...
#usamos seaborn para esta(mas cancheros)
#
...
combustible = pd.read_csv(carpeta+"precioBiodiesel.csv")

# Generar un gráfico para representarlos gráficamente



#plt.scatter(data=combustible, x='Periodo', y='Precio')
...
# Discutir con el resto de la clase
# ¿Cuál fue su objetivo: Explorar, Explicar, Otro? explorar
# ¿Qué tipos de variables estaban en juego? periodo precios
# ¿Qué tipo de gráfico decidió utilizar? grafico lineas- grafico de barras
