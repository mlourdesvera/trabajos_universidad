...
def elementos_exclusivos(s: list, t: list) -> list:
    ...
    for elem in s:
        if elem not in t and elem not in lista_vacia:
            lista_vacia.append(elem)
    for elem in t:
        if elem not in s and elem not in lista_vacia:
            lista_vacia.append(elem)
    return lista_vacia
...
def contar_traducciones_iguales(ingles: dict, aleman: dict) -> int:
    contador:int = 0
    for clave in aleman:
        for clave2 in ingles:
            if (clave == clave2) and (aleman[clave] == ingles[clave2]):
                contador +=1
    return contador
