...
def acomodar(s1: list) -> list:
    ...
    for elem in s1:
        if elem == "LLA":
            lista_lla.append(elem)
        elif elem == "UP":
            lista_up.append(elem)
...
def columnas_repetidas(m: list) -> bool:
    ...
    for columna in m:
        ...
        for i in range (0,int(longitud/2)):
            if columna[i] != columna[i+int(longitud/2)]:
                res = False
    return res
...
def cuenta_posiciones_por_nacion(naciones: list, torneos: dict) -> dict:
    ...
    for year in torneos.keys():
        for i in range (len(torneos[year])):
            res [torneos[year][i]][i] += 1
    return res
